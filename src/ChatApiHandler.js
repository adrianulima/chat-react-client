...
const ChatApiHandler = ({ baseUrl } = {}) => {
  ...
  return {
    ...
    postRoom: (data, config) => {
      return axiosApi.post('/rooms', data, config).then(getResponseData)
    },

    getRooms: (config) => {
      return axiosApi.get('/rooms', config).then(getResponseData)
    },

    getRoom: (roomId, config) => {
      return axiosApi.get(`/rooms/${roomId}`, config).then(getResponseData)
    },
    ...
    deleteRoom: (roomId, config) => {
      return axiosApi.delete(`/rooms/${roomId}`, config).then(getResponseData)
    },
